...
int main(int argc, char* argv[]){
  ...
  //std::cout << "XX " << mass->GetName() << std::endl;
  //reduced->Print("V");
  //
  //for (int i=0; i<reduced->numEntries(); i++) {
  //  const RooArgSet* arg = data->get(i);
  //  // std::cout << "ZZ " << arg->getRealValue(mass->GetName()) << std::endl;
  //  
  //  double m = arg->getRealValue(mass->GetName());
  //  //std::cout << m << std::endl;
  //  if ( m > 110 &&  m<160)
  //    dataBinned->add(*arg, data->weight());
  //}
  //
  ...
  //RooAddPdf *ggh_pdf = (RooAddPdf*)sigWS->pdf("ggH"); //Form("sigpdfsmrel_cat%d_7TeV_ggh",cat));
  //RooRealVar* norm_ggh = new RooRealVar("norm_ggh", "norm_ggh", 8.1134, 0, 10);
  //norm_ggh->setConstant(true);
  //RooExtendPdf* ggh_ext = new RooExtendPdf("ggh_ext", "ggh_ext", *ggh_pdf, *norm_ggh);
  //RooAddPdf *vbf_pdf = (RooAddPdf*)sigWS->pdf("qqH"); //Form("sigpdfsmrel_cat%d_7TeV_vbf",cat));
  //RooRealVar* norm_qqh = new RooRealVar("norm_qqh", "norm_qqh", 0.4879, 0, 10);
  //norm_qqh->setConstant(true);
  //RooExtendPdf* vbf_ext = new RooExtendPdf("vbf_ext", "vbf_ext", *vbf_pdf, *norm_qqh);
  //RooAddPdf *wh_pdf = (RooAddPdf*)sigWS->pdf("WH");  //Form("sigpdfsmrel_cat%d_7TeV_wzh",cat));
  //RooRealVar* norm_wh = new RooRealVar("norm_wh", "norm_wh", 0.2534, 0, 10);
  //norm_wh->setConstant(true);
  //RooExtendPdf* wh_ext = new RooExtendPdf("wh_ext", "wh_ext", *wh_pdf, *norm_wh);
  //RooAddPdf *zh_pdf = (RooAddPdf*)sigWS->pdf("ZH");  //Form("sigpdfsmrel_cat%d_7TeV_tth",cat));
  //RooRealVar* norm_zh = new RooRealVar("norm_zh", "norm_zh", 0.01451, 0, 10);
  //norm_zh->setConstant(true);  
  //RooExtendPdf* zh_ext = new RooExtendPdf("zh_ext", "zh_ext", *zh_pdf, *norm_zh);
  //
  //RooAbsPdf *sigPdf = new RooAddPdf(Form("sigpdfsmrel_cat%d_7TeV",cat),Form("sigpdfsmrel_cat%d_7TeV",cat),RooArgList(*ggh_ext,*vbf_ext,*wh_ext,*zh_ext));
  ...
  for (vector<pair<int,pair<string,string> > >::iterator it=toysMap.begin(); it!=toysMap.end(); it++){
    ...
    //std::cout << "MATTEO: " << it->second.second << " " << it->first << " " << it->second.first.c_str() << std::endl;
  ...
  //  toysModel.setSignalModifierConstant(true);
  toysModel.fitToData(dataBinned, false, true, false);
  //  toysModel.setSignalModifierVal(expectSignal);
  //  toysModel.setSignalModifierConstant(false);
  toysModel.saveWorkspace(outFile);
  if (!skipPlots) 
      toysModel.plotPdfsToData(dataBinned,50,Form("%s/plots/truthToData/datafit_mu%3.1f",outDir.c_str(),expectSignal),false);
  ...
  for (int toy=jobn*ntoys; toy<(jobn+1)*ntoys; toy++){
    ...
    if (!skipPlots) 
      toysModel.plotToysWithPdfs(Form("%s/plots/toys/job%d_toy%d",outDir.c_str(),jobn,toy),50,false);
    //     }
      ...
      else
	{
      ...
      if (!skipPlots)
	testModel.plotPdfsToData(it->second,50,Form("%s/plots/toys/fit_%s",outDir.c_str(),it->first.c_str()),false,"",true,true);
	...
	fitStatus_ = fit->status();
	muTree->Fill();
	fit->SetName(Form("%s_fitResult_toy%d_truth%s",itf->first.c_str(),iToy_,genName_));
	fit->Write();
